...
def calculate_weights(comparison_matrix):
    norm_matrix = comparison_matrix / comparison_matrix.sum(axis=0)
    weights = norm_matrix.mean(axis=1)
    return weights

def consistency_check(comparison_matrix, weights):
    n = comparison_matrix.shape[0]
    weighted_sum = comparison_matrix.dot(weights)
    lambda_max = weighted_sum / weights
    CI = (np.mean(lambda_max) - n) / (n - 1)
    RI = {1: 0, 2: 0, 3: 0.58, 4: 0.90, 5: 1.12, 6: 1.24,
          7: 1.32, 8: 1.41, 9: 1.45, 10: 1.49}
    
    CR = CI / RI[n] if n in RI else CI
    return CR
...
# Exemple de matrice de comparaison (P, S, Ps, R, M)
...
# Exemple d'assignation de valeurs
